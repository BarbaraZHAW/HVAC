...
def ModelRecAir(m, α, θS, θIsp, φIsp, θO, φO, Qsa, Qla, mi, UA):
    ...
    # CC cooling with dehumidification
    A[2, 0], A[2, 2], A[2, 9], b[2] = m * c, - m* c, 1, 0
    A[3, 1], A[3, 3], A[3, 10], b[3] = m * l, - m * l, 1, 0
    A[4, 2], A[4, 3], b[4] = psy.wsp(θs0), -1,\
    psy.wsp(θs0) * θs0 - psy.w(θs0, 1)
    A[5, 8], A[5, 9], A[5, 10], b[5] = -1, 1, 1, 0
    # HC heating coil
    A[6, 2], A[6, 4], A[6, 8], b[6] = m * c, -m * c, 1, 0
    A[7, 3], A[7, 5], b[7] = m * l, -m * l, 0
    # TZ thermal zone
    A[8, 4], A[8, 6], A[8, 10], b[8] = m * c, -m * c, 1, 0
    A[9, 5], A[9, 7], A[9, 11], b[9] = m * l, -m * l, 1, 0
    # BL building
    A[10, 6], A[10, 10], b[10] = (UA + mi * c), 1, (UA + mi * c) * θO + Qsa
    A[11, 7], A[11, 11], b[11] = mi * l, 1, mi * l * wO + Qla
    # Kt indoor temperature controller
    A[12, 6], A[12, 8], b[12] = Kt, 1, Kt * θIsp
    # Kw indoor humidity controller
    A[13, 7], A[13, 9], b[13] = Kw, 1, Kw * wIsp
